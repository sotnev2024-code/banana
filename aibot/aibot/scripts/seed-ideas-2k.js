#!/usr/bin/env node
/**
 * Re-generate all 20 seeded ideas at 2K resolution and replace their media.
 * Uses GPT Image 2 Edit. Matches existing DB rows by exact promptRu — if a
 * row exists, its mediaUrl is updated in place; otherwise a new row is created.
 * Old /uploads/featured/idea_<hash>.webp files are NOT auto-deleted (small
 * footprint, easy to roll back). Categories must already exist.
 *
 * Usage:
 *   KIE_API_KEY=xxxx node scripts/seed-ideas-2k.js
 */
...
// ─── 20 final working prompts (originals 2-7, 9-18, 20 + retry 1, 8, 19) ─────

const IDEAS = [
  // Originally idea #1 (Michelangelo+Nike) → Greek statue + Adidas
  { catSlug: 'concept', refFile: REF(3), badge: 'NEW',
  ...
  { catSlug: 'concept', refFile: REF(3), badge: null,
    ...
    promptEn: `A magazine ad for Adidas featuring an elderly woman with tattoos wearing the classic three stripes. She is wearing a gold and red Adidas sweatshirt and hat that says "to me always". She has one hand up in front of her face, making gang signs like in a typical bank album photo of elderly people doing gang sign hand gestures. The background color is brown. At the top there is text reading "always original". 8K, ultra-detailed, sharp focus, photorealistic.`,
  },
  // 5
  { catSlug: 'streetwear', refFile: REF(5), badge: 'TOP',
  ...
  { catSlug: 'streetwear', refFile: REF(3), badge: null,
    ...
    promptEn: `High-impact streetwear fashion poster, mixed-media editorial campaign, central portrait of the subject from the reference photo, bold urban styling, oversized jacket, layered accessories, strong direct attitude, dramatic studio-meets-street lighting, dark textured background, graphic composition mixing fashion photography with graffiti tags, paint strokes, spray mist, brutal typography blocks, poster tears, street luxury energy, black background with bold orange, acid green, white or violet accents, premium campaign design, sharp face rendering, 35mm lens, ultra-detailed, editorial realism, 8K, photorealistic skin texture, sharp focus. Preserve face identity exactly.`,
  },
  // 8 — replaced cigarette with peace sign
  { catSlug: 'bw-portrait', refFile: REF(1), badge: 'PRO',
  ...
  { catSlug: 'music', refFile: REF(3), badge: null,
    ...
    promptEn: `Predominantly monochromatic palette (black, gray, and white) with subtle neon pink accents for premium visual contrast. Subject: the man from the reference photo wearing premium headphones. Soft, directional lighting with a sharp rim light on the face and metallic highlights on the headphones. Extremely realistic skin texture: visible pores, micro-imperfections, a slight natural glow. Elegant composition with a futuristic music editorial poster or album cover aesthetic. Atmosphere: futuristic, artistic, technological. Aspect ratio 4:5. 8K, ultra-detailed, sharp focus, pure extreme photographic realism. Preserve face identity.`,
  },
  // 11
  { catSlug: 'surreal', refFile: REF(5), badge: 'VIRAL',
    ...
    promptEn: `A surreal urban scene where the subject from the reference photo appears gigantic, leaning forward with hands framing the face, gazing curiously over a modern city intersection below. Extreme scale contrast with tiny cars, pedestrians, and a passing train, rendered in bright daytime light with hyper-real, believable perspective and a dreamy, editorial feel. 8K, ultra-detailed, sharp focus, photorealistic skin texture.`,
  },
  // 12
  { catSlug: 'editorial', refFile: REF(3), badge: null,
    ...
    promptEn: `Use the uploaded photo as the reflected subject. An extreme close-up macro of sunglasses filling the frame, with realistic curvature and distortion, showing the subject's phone selfie outside naturally warped in the lens reflection. Bright outdoor sunlight, detailed skin texture and highlights, with a vintage film / early-digital look, slight grain, and candid summer editorial realism. 8K, ultra-detailed, sharp focus.`,
  },
  // 13
  { catSlug: 'cinematic', refFile: REF(4), badge: 'HOT',
...
async function generate(prompt, refUrl) {
  const r = await kiePost('/jobs/createTask', {
    ...
    input: {
      ...
      input_urls: [refUrl],
      aspect_ratio: '4:5',
      // KIE side: 2K is currently broken for gpt-image-2-image-to-image
      // (always returns "Internal Error"). Use 4K — costs $0.20 vs $0.14
      // but works reliably and gives the maximum quality available.
      resolution: '4K',
      nsfw_checker: true,
    },
  })
...
async function pollDone(taskId, timeoutMs = 10 * 60 * 1000) {
...
function downloadAndCompress(srcUrl, ideaId) {
  ...
  execSync(`curl -s -L -o ${tmpRaw} "${srcUrl}"`, { timeout: 180000 })
  // 4K source → resize to 2000px width @ q88 (~400-700 KB, sharp on retina screens)
  execSync(`cwebp -q 88 -resize 2000 0 ${tmpRaw} -o ${finalPath} 2>/dev/null`, { timeout: 90000 })
  try { fs.unlinkSync(tmpRaw) } catch {}
  return `${PUBLIC_URL}/uploads/featured/${finalName}`
}

async function main() {
  // Categories assumed already present (they were created by the first run)
  const cats = await prisma.ideaCategory.findMany()
  const catBySlug = Object.fromEntries(cats.map(c => [c.slug, c]))

  let ok = 0, failed = 0, updated = 0, created = 0
  for (let i = 0; i < IDEAS.length; i++) {
    const idea = IDEAS[i]
    const tag = `[${i+1}/${IDEAS.length}]`

    try {
      console.log(`${tag} 📤 reference ${idea.refFile}…`)
      const refUrl = await publishReference(idea.refFile)

      console.log(`${tag} 🧠 generating @ 2K…`)
      ...
      if (!cat) { console.error(`${tag} ❌ category ${idea.catSlug} missing`); failed++; continue }

      // Match existing row by exact promptRu — update mediaUrl in place if found
      const existing = await prisma.idea.findFirst({ where: { promptRu: idea.promptRu } })
      if (existing) {
        await prisma.idea.update({
          where: { id: existing.id },
          data: { mediaUrl: localUrl, mediaType: 'image' },
        })
        console.log(`${tag} 🔄 updated existing idea (id=${existing.id})`)
        updated++
      } else {
        ...
        console.log(`${tag} ✨ created new idea (id=${row.id})`)
        created++
      }
      ok++
    } catch (e) {
  ...
  console.log(`\n🎉 Done — ok=${ok} (updated=${updated}, created=${created}), failed=${failed}`)
