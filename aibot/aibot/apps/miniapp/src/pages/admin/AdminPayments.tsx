...
export function AdminPayments() {
  ...
  const statusLabel: Record<string, string> = {
    SUCCEEDED: 'Успешно', PENDING: 'Ожидание', FAILED: 'Ошибка', CANCELLED: 'Отменён',
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      {payments.length === 0 && <div style={{ padding: 20, textAlign: 'center', color: 'var(--text2)' }}>Платежей нет</div>}
        ...
        <div key={p.id} className="card" style={{ padding: 12, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            ...
            <div style={{ fontSize: 11, color: 'var(--text3)' }}>
              {p.tokensGranted} токенов | {p.provider} | {new Date(p.createdAt).toLocaleString('ru')}
            </div>
          </div>
          <div style={{ textAlign: 'right' }}>
            <div style={{ fontSize: 15, fontWeight: 600 }}>{p.amount} {p.currency}</div>
            <div style={{ fontSize: 11, color: statusColor[p.status] ?? 'var(--text3)' }}>{statusLabel[p.status] ?? p.status}</div>
          </div>
        </div>
      ))}
